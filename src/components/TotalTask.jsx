    ...
    <Box>
      <Grid templateColumns="repeat(3, 1fr)" gap="6" alignItems={"center"}>
        <GridItem paddingLeft={"20"} textStyle={"xl"} colSpan={2}>
          <Text color={"blackAlpha.800"} textStyle="xl">
            Task Done
          </Text>
          <Text color={"blackAlpha.900"} textStyle="xl">
            keep it up!
          </Text>
        </GridItem>
        <GridItem colSpan={1}>
          <Box
            ...
            borderRadius={"full"}
          >
            <Text textStyle={"xs"}>{completeTodo?.length || 0}</Text> /{" "}
            <Text textStyle={"xs"}>{todo?.length || 0}</Text>
          </Box>
        </GridItem>
      </Grid>
    </Box>
